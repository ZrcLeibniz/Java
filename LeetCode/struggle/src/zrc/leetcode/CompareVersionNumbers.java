...
class CompareVersionNumbersSolution {
    public static int compareVersion(String version1, String version2) {
        String[] versionI = version1.split("\\.");
        String[] versionII = version2.split("\\.");
        int index = 0;
        while (index < versionI.length || index < versionII.length) {
            String vI = index >= versionI.length ? "0" : versionI[index];
            String vII = index >= versionII.length ? "0" : versionII[index];
            if (vI.equals("") && vII.equals("")) {
                return 0;
            }
            if (vI.equals("") || vII.equals("")) {
                return vI.equals("") ? -1 : 1;
            }
            int result = compare(vI, vII);
            if (result > 0) {
                return 1;
            } else if (result < 0) {
                return -1;
    ...
    private static int compare(String version1, String version2) {
        int I = Integer.parseInt(version1);
        int II = Integer.parseInt(version2);
        return I - II;
    }
